...
def load_processed_data(processed_dir):
    ...
    X_df = pd.read_csv(os.path.join(processed_dir, "customer_segmentation_scaled(1).csv"))
    X = X_df[['Recency', 'Frequency', 'Monetary']].values
    rfm_raw = pd.read_csv(os.path.join(processed_dir, "customer_segmentation(1).csv"))
...
def train_kmeans(X, k, models_dir):
    ...
    model_path = os.path.join(models_dir, "kmeans_model(f).pkl")
...
def predict_full_customers(processed_dir, models_dir, model):
    ...
    print("\n=== DỰ ĐOÁN CỤM CHO TOÀN BỘ KHÁCH HÀNG ===")
    rfm_full = pd.read_csv(os.path.join(processed_dir, "customer_segmentation_full(1).csv"))
    scaler = joblib.load(os.path.join(models_dir, "rfm_scaler(f).pkl"))
...
def export_final_results(rfm_full, processed_dir):
    final_output_path = os.path.join(processed_dir, "customer_segments_final(1).csv")
